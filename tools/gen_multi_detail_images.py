...
def draw_arrow(draw, x, y, color, size=8):
    """画右箭头"""
    draw.polygon([(x, y-size), (x+size, y), (x, y+size)], fill=color)


# ══════════════════════════════════════════════════════════════
# 第1张: 产品主图 + 品牌标题 (800x800)
# ══════════════════════════════════════════════════════════════
def gen_hero_image(name, cat, price, prod_img_path, bg, output_path):
    W, H = 800, 800
    img = Image.new('RGB', (W, H), (255, 255, 255))
    draw = ImageDraw.Draw(img)

    fn = get_font(36); fp = get_font(28); fs = get_font(20); fm = get_font(16)

    # 渐变背景 (上方)
    for y in range(300):
        alpha = 1 - y / 300
        r = int(bg[0] * alpha + 255 * (1 - alpha))
        g = int(bg[1] * alpha + 255 * (1 - alpha))
        b = int(bg[2] * alpha + 255 * (1 - alpha))
        draw.line([(0, y), (W, y)], fill=(r, g, b))

    # 产品主图 (居中)
    if prod_img_path and prod_img_path.exists():
        try:
            prod = Image.open(prod_img_path).convert('RGB')
            prod = crop_center_square(prod, 350)
            img.paste(prod, (225, 180))
        except:
            pass

    # 品牌名
    icon = ICONS.get(cat, '📦')
    draw.text((40, 30), f'{icon} {name}', fill=(255,255,255), font=fn)

    # 价格
    draw.text((40, 560), f'¥{price/100:.0f}', fill=(228, 57, 60), font=fp)

    # 标语
    draw.text((40, 610), '正品保障 · 七天无理由退换', fill=(120,120,120), font=fm)

    # 底部装饰线
    draw.rectangle([0, H-4, W, H], fill=bg)
...
# 第2张: 功能卖点卡片 (800x600)
# ══════════════════════════════════════════════════════════════
def gen_features_image(name, desc, bg, output_path):
    W, H = 800, 600
    img = Image.new('RGB', (W, H), (255, 255, 255))
    draw = ImageDraw.Draw(img)

    fn = get_font(28); fs = get_font(20); fm = get_font(16); ft = get_font(14)

    # 标题栏
    draw.rectangle([0, 0, W, 60], fill=bg)
    draw.text((30, 15), '✦ 核心卖点', fill=(255,255,255), font=fn)
    ...
    for i, spec in enumerate(specs[:6]):
        ...
        draw_rounded_rect(draw, (30, y, W-30, y+70), 8, light)

        # 左侧序号圆
        draw.ellipse((50, y+15, 80, y+45), fill=bg)
        draw.text((60, y+20), str(i+1), fill=(255,255,255), font=fs)

        # 卖点文字
        draw.text((95, y+12), spec, fill=(50,50,50), font=fm)

        # 右侧箭头
        draw_arrow(draw, W-50, y+30, bg)

        y += 85

    # 底部品牌条
    draw.rectangle([0, H-40, W, H], fill=bg)
    draw.text((30, H-30), f'MallHub · {name}', fill=(255,255,255), font=fm)
...
# 第3张: 规格参数表 (800x500)
# ══════════════════════════════════════════════════════════════
def gen_specs_image(name, desc, cat, price, bg, output_path):
    W, H = 800, 500
    img = Image.new('RGB', (W, H), (255, 255, 255))
    draw = ImageDraw.Draw(img)

    fn = get_font(28); fs = get_font(18); fm = get_font(15); ft = get_font(14)

    # 标题
    draw.rectangle([0, 0, W, 60], fill=bg)
    draw.text((30, 15), '📋 商品规格', fill=(255,255,255), font=fn)

    # 表头
    draw.rectangle([30, 75, W-30, 110], fill=(245,245,245))
    draw.text((50, 85), '参数名称', fill=(100,100,100), font=fs)
    draw.text((350, 85), '参数详情', fill=(100,100,100), font=fs)

    # 参数行
    specs = [p.strip() for p in re.split(r'[,，、]', desc) if p.strip()]
    y = 115
    for i, spec in enumerate(specs[:7]):
        if i % 2 == 0:
            draw.rectangle([30, y, W-30, y+40], fill=(250,250,250))
        ...
        draw.text((50, y+10), k.strip(), fill=(80,80,80), font=fm)
        draw.text((350, y+10), v.strip(), fill=(50,50,50), font=fm)
        y += 42

    # 额外信息
    draw.line([(30, y+10), (W-30, y+10)], fill=(230,230,230), width=1)
    y += 20
    draw.text((50, y), '商品分类', fill=(80,80,80), font=fm)
    draw.text((350, y), cat, fill=(50,50,50), font=fm)
    y += 35
    draw.text((50, y), '价格', fill=(80,80,80), font=fm)
    draw.text((350, y), f'¥{price/100:.0f}', fill=(228,57,60), font=fm)

    # 底部
    draw.rectangle([0, H-40, W, H], fill=bg)
    draw.text((30, H-30), 'MallHub 正品保障', fill=(255,255,255), font=fm)

    img.save(output_path, 'JPEG', quality=90)


# ══════════════════════════════════════════════════════════════
# 第4张: 品质保障/使用场景 (800x400)
# ══════════════════════════════════════════════════════════════
def gen_trust_image(name, cat, bg, output_path):
    W, H = 800, 400
    img = Image.new('RGB', (W, H), (255, 255, 255))
    draw = ImageDraw.Draw(img)

    fn = get_font(28); fs = get_font(20); fm = get_font(16)

    # 标题
    draw.rectangle([0, 0, W, 60], fill=bg)
    draw.text((30, 15), '🛡️ 品质保障', fill=(255,255,255), font=fn)
    ...
    for icon, title, desc in items:
        # 图标
        draw.text((50, y), icon, fill=bg, font=fs)
        # 标题
        draw.text((90, y), title, fill=(50,50,50), font=fs)
        # 描述
        draw.text((90, y+30), desc, fill=(120,120,120), font=fm)
        y += 70
...
def main():
    ...
    try:
        ...
        for i, p in enumerate(products, 1):
            fname_base = sanitize(p.name)
            prod_img = find_product_image(p.name)
            bg_hex, _ = get_colors(p.name)
            bg = hex_rgb(bg_hex)

            # 生成4张详情图
            for suffix, gen_func, args in [
                ('_hero', gen_hero_image, (p.name, p.category or '', p.price, prod_img, bg)),
                ('_features', gen_features_image, (p.name, p.description or '', bg)),
                ('_specs', gen_specs_image, (p.name, p.description or '', p.category or '', p.price, bg)),
                ('_trust', gen_trust_image, (p.name, p.category or '', bg)),
            ]:
                out = OUTPUT_DIR / f'{fname_base}{suffix}.jpg'
                if not out.exists():
                    gen_func(*args, out)

            # 更新数据库 - 主详情图用 hero 图
            hero_path = f'/static/products/details/{fname_base}_hero.jpg'
            p.detail_image = hero_path
            generated += 1
